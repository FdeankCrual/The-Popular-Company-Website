...
import { LayoutDashboard, CheckSquare, Users, FileText, Database, LogOut } from "lucide-react";

export default async function AdminLayout({ children }: { children: React.ReactNode }) {
  // Read the role passed by middleware
  const headersList = await headers();
  const role = headersList.get("x-user-role") || "CREATOR";
  const email = headersList.get("x-user-email") || "Agent";

  const navItems = [
    { name: "Dashboard", href: "/admin", icon: LayoutDashboard, roles: ["SUPER_ADMIN", "MANAGER", "CREATOR"] },
    { name: "Workbook", href: "/admin/workbook", icon: Database, roles: ["SUPER_ADMIN", "MANAGER", "CREATOR"] },
    { name: "Leads", href: "/admin/leads", icon: Users, roles: ["SUPER_ADMIN", "MANAGER"] },
    { name: "Content", href: "/admin/content", icon: FileText, roles: ["SUPER_ADMIN", "CREATOR"] },
    { name: "Settings", href: "/admin/settings", icon: CheckSquare, roles: ["SUPER_ADMIN", "MANAGER"] },
  ];
      ...
      <aside className="w-full md:w-64 bg-[#0a0a0a] border-r border-white/10 flex flex-col justify-between shrink-0 h-[60px] md:h-full z-20">
        ...
        <div className="hidden md:block p-6 border-t border-white/10">
          <div className="text-xs text-gray-400 font-mono break-all mb-4">
            ...
            <span className="text-tpc-orange">[{role}]</span>
          </div>
