    ...
    <div className="p-4 md:p-12 min-h-dvh">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[20000] flex items-center justify-center p-0 md:p-6 bg-black/90 backdrop-blur-sm">
          <div className="bg-[#111] md:border border-white/10 rounded-none md:rounded-3xl p-6 md:p-8 max-w-lg w-full h-full md:h-auto md:max-h-[90vh] overflow-y-auto relative flex flex-col pt-[calc(1rem+env(safe-area-inset-top))] md:pt-[calc(1.5rem+env(safe-area-inset-top))] pb-[calc(1.5rem+env(safe-area-inset-bottom))]">
            <button onClick={() => { setIsModalOpen(false); setEditingId(null); }} className="absolute top-4 right-4 md:top-6 md:right-6 text-gray-500 hover:text-white bg-black/50 md:bg-transparent p-2 md:p-0 rounded-full z-10">
              <X className="w-5 h-5" />
            </button>
